...
class SingleFileExtractor(IExtractor):
    ...
    def _load_signals(self, file_path):
        try:
            with h5py.File(file_path, "r") as hdf:
                ...
                # data = hdf["summaries"]["minutes"]
                # print(data.dtype.names)
                # column_names = data.dtype.names
                # data_dict = {col: data[col] for col in column_names}
                # print(data_dict, data.dtype.names)
                # data = data_dict["PRx"]
                # # Drop NaNs at the start and end of the data
    ...
    def cohen_kappa_matrix(self, mode, include_good=True):
        """
        Computes Cohen's Kappa score for the specified mode between all pairs of annotators.
        Returns a tuple containing:
            - A sorted list of annotator names.
            - A matrix (numpy array) where matrix[i, j] is the Cohen's Kappa score 
              between annotator i and annotator j.
        """
        mode_annotations = self.get_annotations(mode)
        if not mode_annotations:
             raise ValueError(f"No annotations found for mode {mode}")

        annotators = sorted({annotation.annotator for annotation in mode_annotations.values()})
        
        if len(annotators) < 2:
            return annotators, np.array([[1.0]]) if len(annotators) == 1 else np.array([])

        segment_ratings = defaultdict(dict)
        all_segment_ids = set()

        for annotation in mode_annotations.values():
            annotator = annotation.annotator
            for segment in annotation.good_segments:
                segment_ratings[segment.id][annotator] = 0 # 0 for good
                all_segment_ids.add(segment.id)
            for segment in annotation.anomalies:
                segment_ratings[segment.id][annotator] = 1 # 1 for anomalous
                all_segment_ids.add(segment.id)
        
        segments_to_consider = sorted(list(all_segment_ids))

        kappa_matrix = np.full((len(annotators), len(annotators)), np.nan)
        ...
        for i, annotator_i in enumerate(annotators):
            for j, annotator_j in enumerate(annotators):
                if i == j:
                    ...
                    continue
                if not np.isnan(kappa_matrix[j, i]): # Already computed
                    kappa_matrix[i, j] = kappa_matrix[j, i]
                    continue
                ...
                for segment_id in segments_to_consider:
                    rating_i = segment_ratings[segment_id].get(annotator_i)
                    rating_j = segment_ratings[segment_id].get(annotator_j)

                    if rating_i is not None and rating_j is not None:
                        ratings_i.append(rating_i)
                        ratings_j.append(rating_j)
                
                if len(ratings_i) < 2: 
                    kappa_score = np.nan
                else:
                    try:
                        kappa_score = cohen_kappa_score(ratings_i, ratings_j)
                    except ValueError: # Handle potential issues if sklearn raises error
                        kappa_score = np.nan

                kappa_matrix[i, j] = kappa_score
        
        return annotators, kappa_matrix
...
class FolderExtractor(IExtractor):
    ...
    def compute_cohen_kappa(self, mode):
        """
        Computes an aggregated Cohen's Kappa matrix across all files for the specified mode.
        Returns a tuple containing:
            - A sorted list of unique annotator names across all files.
            - A matrix (numpy array) where matrix[i, j] is the average Cohen's Kappa score 
              between annotator i and annotator j across all relevant files.
        """
        ...
        for extractor in self._extractors:
            try:
                annotators, kappa_matrix = extractor.cohen_kappa_matrix(mode)
                all_annotators.update(annotators)

                for i, annotator_i in enumerate(annotators):
                    for j, annotator_j in enumerate(annotators):
                        if i < j: # Process each pair once
                            score = kappa_matrix[i, j]
                            if not np.isnan(score):
                                pair = tuple(sorted((annotator_i, annotator_j)))
                                aggregated_scores[pair].append(score)

            except ValueError as e:
                print(f"Skipping Kappa calculation for mode {mode} in {extractor.hdf5_file_stem}: {e}")

        average_scores = {pair: np.mean(scores) for pair, scores in aggregated_scores.items() if scores}

        unique_annotators = sorted(list(all_annotators))
        num_annotators = len(unique_annotators)
        final_kappa_matrix = np.full((num_annotators, num_annotators), np.nan) # Use NaN for missing pairs

        for i, annotator_i in enumerate(unique_annotators):
            for j, annotator_j in enumerate(unique_annotators):
                if i == j:
                    final_kappa_matrix[i, j] = 1.0
                else:
                    pair = tuple(sorted((annotator_i, annotator_j)))
                    avg_score = average_scores.get(pair)
                    if avg_score is not None:
                         final_kappa_matrix[i, j] = avg_score

        return unique_annotators, final_kappa_matrix
